...
useEffect(()=>{
    ...
    if(registros.length >= 1){
       setLoading(true) 
       //llamar al api para enviarle los datos del usuario que si intenta registrar
    //const res = fetch( bla );

    //poner alguna visualizacion que indique demora (spinner, progress bar,)
    //setear un estado intermedio de tu com´ponente mientras manda data.
    setTimeout( () => {
        setLoading(false)
        setRespuesta(true)
        
        //si la respuestas esta ok, mostrar mensaje de que te se creo con exito y en el mensaje poner link de login
        
        //si la rspuesta esta no ok. mostrar errores, y pintar de rojo el formulario
        //if(res.error = true){
        ...
        <div className="form-registrarse">
            <form id="formularioSignUp" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="inputs contraseña">
                    ...
                    {errors.contraseña?.type === 'pattern' && <p className="p-validacion">La contraseña debe tener entre 8 y 16 caracteres, al menos un dígito, al menos una minúscula y al menos una mayúscula.
